...
(function (global, factory) {
      ...
      return {
          ...
          animationStates.forEach(state => {
            ...
            if (target.thisAnimationDuration) {
              ...
              (animatingRect.top - toRect.top) / (animatingRect.left - toRect.left) === (fromRect.top - toRect.top) / (fromRect.left - toRect.left)) {
                ...
                time = calculateRealTime(animatingRect, prevFromRect, prevToRect, sortable.options);
              }
            }
        ...
        animate: (target, currentRect, toRect, duration) => {
          if (duration) {
            ...
            css(target, "transition", "transform " + duration + "ms" + (sortable.options.easing ? " " + sortable.options.easing : ""));
  ...
  const onMove = (fromEl, toEl, dragEl, dragRect, targetEl, targetRect, originalEvent, willInsertAfter) => {
    let evt,
  ...
  const _disableDraggable = el => {
    el.draggable = false;
  };
  const _ghostIsFirst = (evt, vertical, sortable) => {
    let rect = getRect(getChild(sortable.el, 0, sortable.options, true));
    const spacer = 10;
    return vertical ? evt.clientX < rect.left - spacer || evt.clientY < rect.top && evt.clientX < rect.right : evt.clientY < rect.top - spacer || evt.clientY < rect.bottom && evt.clientX < rect.left;
  };
  const _ghostIsLast = (evt, vertical, sortable) => {
    let rect = getRect(lastChild(sortable.el, sortable.options.draggable));
    const spacer = 10;
    return vertical ? evt.clientX > rect.right + spacer || evt.clientX <= rect.right && evt.clientY > rect.bottom && evt.clientX >= rect.left : evt.clientX > rect.right && evt.clientY > rect.top || evt.clientX <= rect.right && evt.clientY > rect.bottom + spacer;
  };
  const _getSwapDirection = (evt, target, targetRect, vertical, swapThreshold, invertedSwapThreshold, invertSwap, isLastTarget, dragEl, targetMoveDistance, pastFirstInvertThresh, lastDirection) => {
   ...
   */
  const _getInsertDirection = (target, dragEl) => {
  ...
  const _saveInputCheckedState = (root, savedInputChecked) => {
  ...
  class Sortable {
    constructor(el, options) {
      ...
      _extends(this, new AnimationStateManager(this));
    }
    ...
    _onDragOver( /**Event*/evt) {
      ...
      : putSortable === this || (this.lastPutMode = activeGroup.checkPull(this, activeSortable, dragEl, evt)) && group.checkPut(this, activeSortable, dragEl, evt))) {
        ...
        if (!elLastChild || _ghostIsLast(evt, vertical, this) && !elLastChild.animated) {
          ...
          direction = _getSwapDirection(evt, target, targetRect, vertical, differentRowCol ? 1 : options.swapThreshold, options.invertedSwapThreshold == null ? options.swapThreshold : options.invertedSwapThreshold, isCircumstantialInvert, lastTarget === target, dragEl, targetMoveDistance, pastFirstInvertThresh, lastDirection);
          let sibling;
          if (direction !== 0) {
